...
from utils.utils_head import bbox_iou, merge_bboxes
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        # input为bs,3*(5+num_classes),13,13  预测结果；targets为真实框
        ...
        # print(input.size()) #[2, 255, 13, 13]
        prediction = input.view(bs, int(self.num_anchors / 3),
                                self.bbox_attrs, in_h,
                                in_w).permute(0, 1, 3, 4, 2).contiguous()

        # 对prediction预测进行调整
        conf = torch.sigmoid(prediction[..., 4])  # Conf
        pred_cls = torch.sigmoid(prediction[..., 5:])  # Cls pred. 先验框内部物体的种类
